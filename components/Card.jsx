...
const Card = ({ question, answer }) => {
    ...
    return (
        <div className="flex-1"> {/* Add flex properties to make each card independent */}
            <div 
                className={`bg-white p-4 rounded-lg shadow-xl transition-all ease-in-out ${isOpen ? 'mb-6' : 'mb-4'} hover:bg-gray-10 cursor-pointer`}
                onClick={() => setIsOpen(!isOpen)}
                style={{ 
                    minHeight: isOpen ? 'auto' : '110px', 
                    transitionDuration: isOpen ? '1000ms' : '500ms' // Slower when opening, faster when closing
                }}
            >
                <div>
                ...
                <div className={`mt-4 text-gray-600 font-raleway transition-all ease-in-out ${isOpen ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}
                    style={{ transitionDuration: isOpen ? '1200ms' : '500ms' }}
                >
                    {answer}
                </div>
            </div>
